...
function getMnyamaShopConfig() {
  const projectId =
    process.env.MNYAMA_SHOP_FIREBASE_PROJECT_ID ??
    process.env.FIREBASE_PROJECT_ID ??
    DEFAULT_PROJECT_ID
  const apiKey = process.env.MNYAMA_SHOP_FIREBASE_API_KEY ?? process.env.FIREBASE_API_KEY

  if (!apiKey) {
    throw new Error('FIREBASE_API_KEY or MNYAMA_SHOP_FIREBASE_API_KEY is not configured')
  }
